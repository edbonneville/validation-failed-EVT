...
```{r updating}
mod_coefs
log_recal <- glm(imps_valid$Mc_FailedFemoralApproach ~ lp_valid, family = binomial())
coef(log_recal)

news_coefs <- c(coef(log_recal)[[1]], coef(log_recal)[[2]] * drop(mod_coefs)[-1])
new_lp <- drop((news_coefs) %*% t(cbind(1, X_valid)))

df_calplot <- cbind.data.frame("lp" = new_lp, imps_valid)
knots <- 4
...
stacked_df <- data.frame(
    "predicted" = unique(plogis(new_lp)),
    "observed" = predict(mod_cal_stacked, 
                         newdata = data.frame("lp" = unique(plogis(new_lp))), 
                         type = "response")
)
plot(stacked_df$predicted, stacked_df$observed)
```
